...
def _load_late_fusion_decoder_head_warmstart(model, fusion_cfg):
    if not fusion_cfg.get("load_beats_decoder_head", False):
        return

    ckpt_path = fusion_cfg.get("beats_decoder_head_ckpt", "")
    if not ckpt_path:
        print(
            "[late-fusion warm-start] enabled, but "
            "model.fusion.beats_decoder_head_ckpt is empty; skipping."
        )
        return
    ...
    if not ckpt_path.is_file():
        print(
            f"[late-fusion warm-start] checkpoint not found: {ckpt_path}; skipping."
        )
        return
    ...
    if source_prefix is None:
        print(
            "[late-fusion warm-start] no compatible decoder prefix found in "
    ...
    # The late-fusion decoder shares the BiGRU + strong/weak heads with the
    # BEATs-only model, while merge MLP remains late-fusion-specific.
    target_prefixes = (
        "decoder.temporal.",
        "decoder.strong_head.",
    ...
    for target_key, target_tensor in target_state.items():
        ...
        if tuple(source_tensor.shape) != tuple(target_tensor.shape):
            mismatched_keys.append(
                (target_key, source_key, tuple(target_tensor.shape), tuple(source_tensor.shape))
            )
            continue
    ...
    print(
        "[late-fusion warm-start] loaded decoder/head from "
        f"{ckpt_path} using prefix {source_prefix}"
    )
    print(
        "[late-fusion warm-start] loaded "
    ...
    for target_key in skipped_target_keys:
        print(
            ...
            "(late-fusion uses merge MLP output, so decoder.input_proj stays untouched)"
        )
    ...
    if relevant_missing_keys:
        print(
            "[late-fusion warm-start] load_state_dict missing keys "
            f"(within decoder/head scope): {relevant_missing_keys}"
        )
    if unexpected_keys:
        print(
            "[late-fusion warm-start] load_state_dict unexpected keys: "
            f"{unexpected_keys}"
        )
...
def build_sed_model(config):
    ...
    if model_type == "crnn_beats_late_fusion":
        ...
        _load_late_fusion_decoder_head_warmstart(model, fusion_cfg)
        return model
